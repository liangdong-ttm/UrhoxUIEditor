...
仓库：https://github.com/liangdong-ttm/UrhoxUIEditor
...
- `python3 -m http.server`（没有 `/api/save`）
...
添加 **图片** 后会进入选图：底部切到「项目」，点一张项目图，或把项目图拖到右侧图片框 / 画布。
...
Lua 写死的 UI 抽成 json：UrhoX 仓库 skill `lua-ui-to-json`。
...
2. 底部 **UI配置** 能列出上述 json。点 `start.ui.json` 中间有图。
...
- 不是完整 Yoga WASM，复杂 Flex 可能和游戏有像素差
